...
export default function UniformPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const [countriesData, categoriesData, uniformsData] = await Promise.all([
        adminApi.getAllCountries(),
        adminApi.getAllCategories(),
        selectedCategory 
          ? adminApi.getUniformsByCategory(selectedCategory)
            ...
            <button
              ...
              className={`p-8 rounded-3xl border-4 transition-all transform hover:scale-105 text-left ${
                selectedCategory === cat.id
                  ? "bg-brand-red text-white border-brand-red shadow-pop-hover"
                  : "bg-white text-brand-text border-brand-yellow/30 shadow-pop hover:shadow-pop-hover"
              }`}
            >
              <h3 className="font-black text-2xl mb-3">{cat.categoryName}</h3>
              <p className={`text-sm font-bold ${selectedCategory === cat.id ? "opacity-90" : "opacity-70"}`}>
                {cat.description || "Chi tiết"}
              </p>
            </button>
          ))}
        </div>
      ...
      <section className="py-12 container-page">
            ...
            <div className="flex gap-4 items-center flex-wrap">
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  className="px-4 py-3 border-2 border-brand-red/30 rounded-xl text-lg focus:ring-4 focus:ring-brand-yellow focus:border-brand-yellow outline-none bg-white font-bold"
                >
              ...
              <div className="flex gap-2 bg-brand-bg rounded-xl p-1">
                <button
                  ...
                  className={`p-2 rounded-lg transition-all ${viewMode === "grid" ? "bg-brand-red text-white" : "text-brand-text/60 hover:text-brand-red"}`}
                >
                  <Grid size={20} />
                </button>
                <button
                  onClick={() => setViewMode("list")}
                  className={`p-2 rounded-lg transition-all ${viewMode === "list" ? "bg-brand-red text-white" : "text-brand-text/60 hover:text-brand-red"}`}
                >
                  <List size={20} />
                </button>
              </div>
          ...
          <div className="mt-6 flex flex-wrap gap-3 text-sm">
            ...
            {selectedCountry && (
              <button
                ...
                className="px-3 py-1 bg-brand-red/10 text-brand-red rounded-full hover:bg-brand-red hover:text-white transition-colors"
              >
                ✕ Xóa bộ lọc quốc gia
              </button>
            )}
        ...
        {loading ? (
              ...
              <Link key={item.id} to={`/tai-lieu/quan-trang/${item.id}`} className="group">
                <div className="bg-white rounded-4xl p-4 border-4 border-brand-bg shadow-pop group-hover:shadow-pop-hover group-hover:-translate-y-1 transition-all duration-300 h-full flex flex-col">
                  <div className="aspect-[4/3] rounded-3xl overflow-hidden border-2 border-brand-bg bg-brand-bg/50 relative">
                    ...
                    {item.country && (
                      <div className="absolute top-3 left-3 bg-brand-yellow text-brand-redDark text-xs font-black px-3 py-1 rounded-full">
          ...
          <div className="space-y-6">
            {filteredItems.map((item) => (
              <Link key={item.id} to={`/tai-lieu/quan-trang/${item.id}`} className="group block">
                  ...
                  <div className="flex gap-6">
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between gap-4">
                        ...
                        <span className="bg-brand-red text-white w-10 h-10 flex items-center justify-center rounded-full font-black group-hover:bg-brand-yellow group-hover:text-brand-redDark flex-shrink-0">
                          →
                        </span>
                      </div>
